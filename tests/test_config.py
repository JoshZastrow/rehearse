...
def test_interactive_endpoint_config_default(
    ...
    assert cfg.interactive_provider_endpoint == ""
...
def test_interactive_endpoint_config_from_env(
    ...
    _set_required_env(monkeypatch, tmp_path)
    monkeypatch.setenv("INTERACTIVE_PROVIDER_ENDPOINT", "wss://example.modal.run/ws")
    from rehearse.config import RuntimeConfig

    cfg = RuntimeConfig.from_env(load_dotenv_file=False)
    assert cfg.interactive_provider_endpoint == "wss://example.modal.run/ws"
