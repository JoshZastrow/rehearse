...
_WORDS = [
    {"text": "Hello",   "start": 0.5,  "end": 0.9},   # inside SPEAKER_00 / coach
    {"text": "there",   "start": 1.0,  "end": 1.3},   # inside SPEAKER_00 / coach
    {"text": "Yes",     "start": 4.0,  "end": 4.3},   # inside SPEAKER_01 / user
    {"text": "exactly", "start": 8.0,  "end": 8.5},   # inside SPEAKER_00 / coach
]


# ─── _map_speaker_ids_to_roles ────────────────────────────────────────────────

def test_map_ids_uses_first_coach_utterance_as_anchor():
    id_to_role = _map_speaker_ids_to_roles(_SEGMENTS, _TRANSCRIPT)
    # First coach utterance is 0.0–3.0, which overlaps SPEAKER_00
    assert id_to_role["SPEAKER_00"] == "coach"
    assert id_to_role["SPEAKER_01"] == "user"
...
def test_assign_from_segments_correct_labels():
    ...
    assert labels == ["coach", "coach", "user", "coach"]


def test_assign_from_segments_no_overlap_defaults_to_user():
    words = [{"text": "gap", "start": 3.1, "end": 3.2}]  # between segments
    id_to_role = {"SPEAKER_00": "coach", "SPEAKER_01": "user"}
...
def test_assign_from_transcript_no_match_defaults_to_user():
    words = [{"text": "silence", "start": 11.0, "end": 11.5}]  # past all utterances
    labels = _assign_speakers_from_transcript(words, _TRANSCRIPT)
    assert labels == ["user"]


def test_assign_from_transcript_unknown_speaker_defaults_to_user():
