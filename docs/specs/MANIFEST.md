...
| [`v2026-05-13-pipeline-event-bus.md`](v2026-05-13-pipeline-event-bus.md) | `draft` | `implementation` | Outbox event bus for audio pipeline choreography | File-backed outbox + asyncio.Queue fan-out. `finalize()` publishes `session_finalized`; VAD worker emits `vad_complete`; enhancement worker emits `enhancement_complete`. No orchestrator. Structured logging at every event boundary. |
| [`v2026-05-13-vad-turn-segmentation.md`](v2026-05-13-vad-turn-segmentation.md) | `draft` | `implementation` | Voice training pipeline step 1 — per-clip audio export | Slices `audio/user/turn_*.wav` at `timing.jsonl` VAD boundaries into 3–30s clips aligned with transcript text. Emits `AudioClipRecord` rows to `pipeline/clips/clips.jsonl`. Triggered by `session_finalized` event via the pipeline bus. |
| [`v2026-05-13-audio-source-separation.md`](v2026-05-13-audio-source-separation.md) | `draft` | `implementation` | Voice training pipeline step 2 — source separation + 24kHz enhancement | Denoises and bandwidth-extends clips from step 1 using `resemble-enhance`. DNSMOS ≥ 3.0 quality gate. Emits `EnhancedClipRecord` rows and final `VoiceTrainingRecord` `(wav_path, text)` pairs to `pipeline/enhanced/`. Triggered by `vad_complete` event via the pipeline bus. |
| [`v2026-05-14-conversation-backend.md`](v2026-05-14-conversation-backend.md) | `wip` | `implementation` | Backend abstraction layer — `ConversationBackend` protocol, `ManagedBackend`, `PipelineBackend`, telephony refactor | Full implementation on `feat/conversation-backend` branch: protocol, factory, `ManagedBackend` (wraps Hume), `PipelineBackend` (energy VAD + Whisper + TTSService), `BusPublisher` frame translator, `ProsodyService` protocol + impls, integration tests. Not yet merged to main. |
| [`v2026-05-18-meeting-phase-processor.md`](v2026-05-18-meeting-phase-processor.md) | `acknowledged` | `implementation` | LLM-driven phase transitions — `MeetingPhaseProcessor` | Replaces heuristic `PhaseProcessor` with a Haiku-class classifier that calls `transition_phase` tool. Bus subscriber pattern unchanged. Reason field written to telemetry as learning signal. Toggled via `enable_meeting_phase_processor` config flag. |
| [`v2026-05-29-interactive-modal-inference.md`](v2026-05-29-interactive-modal-inference.md) | `wip` | `implementation` | Model-agnostic interactive naming + Modal inference server + PersonaPlex support | Renames `MOSHI_*` → `INTERACTIVE_*`; renames `MoshiBackend` → `InteractiveBackend`; adds `BACKEND_TYPE=interactive-modal` (ModalInteractiveBackend WebSocket client); Modal server design for Moshi + PersonaPlex; PersonaPlex persona priming via `swap_persona()`. **Shipped**: naming rename (config, factory, app, shims, tests). **Open**: ModalInteractiveBackend, Modal server, PersonaPlex support. |
...
| ML data pipeline (voice TTS) | `v2026-05-13-pipeline-event-bus.md`, `v2026-05-13-vad-turn-segmentation.md`, `v2026-05-13-audio-source-separation.md` | Read bus spec first — it defines how workers are triggered and wired. Workers run in order: vad_segment (asyncio task) → audio_enhance (ProcessPoolExecutor or separate process). |
